...
type NewItemProps = Omit<ItemProps, "id">;
...
function Page() {
  ...
  const handleAddItem = async (newItem: NewItemProps) => {
    if (!user?.uid) return;
    const id = await addItem(user.uid, newItem);
    setItems((prevItems) => [...prevItems, { ...newItem, id }]);
  };
  const handleItemSelect = (item: NewItemProps) => {
  ...
  return (
    <main className="justify-center items-center">
      ...
      <div className="flex flex-row gap-10">
        ...
        >
          Home Page      </Link>
        <button onClick={() => router.push("../week-10")} className=" bg-amber-700  p-5 text-white rounded-2xl cursor-pointer hover:opacity-40 animate-bounce ">Go Back</button>
      </div>
